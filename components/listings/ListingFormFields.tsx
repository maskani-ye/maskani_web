...
// ─── حالة نموذج الإعلان (كل الحقول النصية كسلاسل) ───────────────────────────
...
interface Props {
  ...
  setField: <K extends keyof ListingFormState>(key: K, value: ListingFormState[K]) => void;
  cities: City[];
  propertyTypes: PropertyTypeItem[];
}

// ─── حقول نموذج الإعلان (مشترك بين إنشاء وتعديل) ────────────────────────────
export function ListingFormFields({ form, setField, cities, propertyTypes }: Props) {
  return (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {FEATURE_TOGGLES.map(({ key, label, icon: Icon }) => {
            const active = form[key as keyof ListingFormState] as boolean;
            return (
              <button
                key={key}
                type="button"
                onClick={() => setField(key as keyof ListingFormState, !active as never)}
...
// multipart لإنشاء إعلان (مع الصور)
export function buildListingFormData(form: ListingFormState, images: File[]): FormData {
  ...
  BOOLEAN_KEYS.forEach((k) => fd.append(k, String(form[k as keyof ListingFormState])));
  images.forEach((img) => fd.append("images", img));
  return fd;
}

// JSON لتعديل إعلان (PATCH) — القيم الرقمية الفارغة تصبح null
export function buildListingPatch(form: ListingFormState): Record<string, unknown> {
  ...
  BOOLEAN_KEYS.forEach((k) => { payload[k] = form[k as keyof ListingFormState]; });
  return payload;
}
