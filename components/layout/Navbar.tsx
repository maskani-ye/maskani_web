      ...
      <div className="w-full px-3 sm:px-5 lg:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2 flex-shrink-0">
            ...
            {/* ⚠️ **النسخة الأفقية للترويسة.** المرصوف رأسياً (رمزٌ فوق اسمٍ فوق
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active ? "bg-primary/10 text-primary" : "text-muted-600 hover:bg-muted-100 hover:text-ink"
                  )}
                >
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <Link
              ...
              className="flex items-center gap-1.5 rounded-xl bg-gold px-3.5 py-2 text-body font-bold text-ink hover:bg-gold/90 transition-colors"
            >
            ...
            {!countryLoading && countries.length > 1 && (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-muted-100 text-body text-muted-700 transition-colors"
                ...
                {countryOpen && (
                  <div className="absolute left-0 mt-2 w-52 max-h-80 overflow-y-auto bg-white rounded-2xl shadow-xl border border-muted-100 py-2 z-50">
                    {countries.map((c) => (
                      <button
                        key={c.id}
                        onClick={() => switchMarket(c)}
                        className={cn(
                          "flex items-center gap-2 px-4 py-2.5 hover:bg-muted-50 text-body w-full text-right",
                          c.code === countryCode ? "text-primary font-bold" : "text-muted-700",
                        )}
                      >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl hover:bg-muted-100 transition-colors text-body font-semibold text-muted-700"
                ...
                <div className="absolute left-0 mt-2 w-52 max-h-80 overflow-y-auto bg-white rounded-2xl shadow-xl border border-muted-100 py-2 z-50">
                  {cities.map((c) => (
                    <button
                      ...
                      className={cn(
                        "flex items-center gap-2 px-4 py-2.5 hover:bg-muted-50 text-body w-full text-right",
                        String(c.id) === cityId ? "text-primary font-bold" : "text-muted-700"
                      )}
                    >
                      {c.name_ar}
                    </button>
            ...
            {user ? (
              <>
                ...
                {/* Notifications */}
                <Link href="/notifications" aria-label="الإشعارات" className="relative p-2 rounded-lg hover:bg-muted-100 transition-colors">
                  <Bell className="h-5 w-5 text-muted-600" />
                </Link>
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-muted-100 transition-colors"
                  >
                    {user.avatar ? (
                    ...
                    <span className="text-body font-semibold text-ink max-w-24 truncate">{user.full_name}</span>
                    <AltArrowDown className="h-4 w-4 text-muted" />
                  </button>

                  {profileOpen && (
                    <div className="absolute left-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-muted-100 py-2 z-50">
                      <Link href="/profile" className="flex items-center gap-2 px-4 py-2.5 hover:bg-muted-50 text-body text-muted-700" onClick={() => setProfileOpen(false)}>
                        <User className="h-4 w-4 text-primary" /> ملفي الشخصي
                      </Link>
                      <Link href="/favorites" className="flex items-center gap-2 px-4 py-2.5 hover:bg-muted-50 text-body text-muted-700" onClick={() => setProfileOpen(false)}>
          ...
          {!countryLoading && countries.length > 1 && (
            <div className="relative lg:hidden">
              <button
                onClick={() => setCountryOpen((v) => !v)}
                className="flex items-center gap-1 p-2 rounded-lg hover:bg-muted-100 transition-colors"
                aria-haspopup="listbox"
                aria-expanded={countryOpen}
                aria-label="تغيير الدولة"
              >
          ...
          {/* Mobile Menu Button */}
          <button
            className="lg:hidden p-2 rounded-lg hover:bg-muted-100"
        ...
        {/* Mobile Menu */}
        {mobileOpen && (
          <div className="lg:hidden border-t border-muted-100 py-3 space-y-1">
            ...
            {countries.length > 1 && (
              <div className="px-4 pb-3">
                ...
                <select
                  ...
                  className="w-full rounded-xl border border-muted-200 bg-white px-3 py-2 text-body text-ink focus:border-primary focus:outline-none"
                >
                  {countries.map((c) => (
            ...
            <div className="px-4 pb-2">
              <label className="flex items-center gap-1.5 text-caption font-semibold text-muted-500 mb-1">
              ...
              <select
                ...
                className="w-full rounded-xl border border-muted-200 bg-white px-3 py-2 text-body text-muted-700 focus:border-primary focus:outline-none"
              >
            ...
            {navLinks.filter((l) => l.href === "/reports").map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-body font-medium text-muted-700 hover:bg-muted-50"
            ...
            <Link
              href="/properties/create"
              className="flex items-center gap-2 mx-4 mt-2 rounded-xl bg-gold px-4 py-2.5 text-body font-bold text-ink"
              onClick={() => setMobileOpen(false)}
            >
              <AddCircle className="h-4 w-4" />
              أضف عقارك مجاناً
