  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="bg-white rounded-2xl card-shadow p-6">
        ...
        <h1 className="text-xl font-bold text-gray-900 mb-3">{request.title || `${request.client_name} يبحث عن عقار`}</h1>
      ...
      <div className="bg-white rounded-2xl card-shadow p-6 mt-6">
        ...
        {!matchesLoaded ? (
          ...
          <p className="text-sm text-gray-400 mt-3">لا توجد عقارات مطابقة متاحة حالياً.</p>
        ) : (
                  ...
                  {offer.property_details && (
                    <Link href={`/properties/${offer.property_details.id}`} className="flex items-center gap-2 bg-cream rounded-xl p-2 mb-2 hover:bg-primary/5 transition-colors">
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                        {offer.property_details.main_image ? <img src={offer.property_details.main_image} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center"><Buildings2 className="h-5 w-5 text-gray-300" /></div>}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-800 truncate">{offer.property_details.title}</p>
                        <p className="text-xs text-primary font-bold">{formatPrice(offer.property_details.price, offer.property_details.currency)}</p>
                      </div>
                    </Link>
                  )}
                  {offer.contact_phone && (
