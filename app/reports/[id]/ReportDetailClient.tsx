...
export default function ReportDetailClient({ id, initialReport }: { id: string; initialReport: FraudReport | null }) {
  const router = useRouter();
  const { user } = useAuth();
  const { requireAuth } = useAuthGate();
  const [report, setReport] = useState<FraudReport | null>(initialReport);
