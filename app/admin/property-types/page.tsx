    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <PageHeader icon={<Widget />} title="أنواع العقارات"
          subtitle={`${total.toLocaleString(NUMERIC_LOCALE)} نوع`} />
            ...
            <tbody className="divide-y divide-muted-50">
              {types.map((t) => (
                <tr key={t.id} className="hover:bg-muted-50/50 transition-colors">
                  <td className="py-3 px-4 text-muted">{t.order}</td>
                  ...
                  <td className="py-3 px-4 font-semibold text-ink">
                    ...
                    {typeof t.properties_count === "number" && (
                      <span className="text-caption text-muted font-normal mr-2">({t.properties_count} عقار)</span>
                    )}
                  </td>
                  <td className="py-3 px-4 text-muted-500 hidden sm:table-cell">{t.name_en || "—"}</td>
                  <td className="py-3 px-4 hidden md:table-cell">
                    <span className="font-mono text-caption bg-muted-100 px-2 py-1 rounded">{t.slug || "—"}</span>
                  </td>
                  <td className="py-3 px-4">
