...
interface AdminProperty {
  ...
  created_at: string;
  ai_risk?: string; // '' | low | medium | high
  ai_flags?: string[];
}
...
export default function AdminPropertiesPage() {
  ...
  const toggleActive = async (l: AdminProperty) => {
    try {
      const res = await api.patch<AdminProperty>(ep.admin.property(l.id), { is_active: !l.is_active });
      const updated = res.data;
      setProperties((prev) => prev.map((x) => x.id === updated.id ? updated : x));
      if (selected?.id === updated.id) setSelected(updated);
      toast.success(updated.is_active ? "تم تفعيل العقار" : "تم إيقاف العقار");
    } catch (err) { toast.error(getErrorMessage(err)); }
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      // Soft delete: backend returns 200 with the deactivated property body.
      const res = await api.delete<AdminProperty>(ep.admin.property(deleteTarget.id));
      const updated = res.data ?? { ...deleteTarget, is_active: false };
      toast.success("تم إيقاف العقار");
      setProperties((prev) => prev.map((x) => x.id === deleteTarget.id ? updated : x));
  ...
  return (
    <div className="px-4 sm:px-6 py-8">
          ...
          {total > LIMIT && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" disabled={offset === 0} onClick={() => fetchProperties(offset - LIMIT)}>السابق</Button>
                <Button size="sm" variant="outline" disabled={offset + LIMIT >= total} onClick={() => fetchProperties(offset + LIMIT)}>التالي</Button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="w-72 shrink-0 bg-white rounded-2xl card-shadow p-5 self-start sticky top-6">
            ...
            <div className="space-y-2">
              <Button onClick={() => toggleActive(selected)} variant="outline" fullWidth size="sm">
                {selected.is_active
                  ? <><DangerCircle className="h-4 w-4" /> إيقاف العقار</>
                  : <><CheckCircle className="h-4 w-4" /> تفعيل العقار</>}
              </Button>
      ...
      <ConfirmDialog
        ...
        message={
          deleteTarget
            ? <>هل أنت متأكد من إيقاف عقار <strong>{deleteTarget.title}</strong>؟</>
            : ""
        }
        confirmLabel="إيقاف"
        variant="danger"
