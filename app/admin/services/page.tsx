        ...
        <div className="flex-1 bg-white rounded-2xl card-shadow overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-muted">
              ...
              <p className="text-body">لا يوجد مزودو خدمة</p>
            </div>
          ) : (
            <table className="w-full text-body">
              <thead className="bg-muted-50 border-b border-muted-100">
                <tr>
                  <th className="text-right px-4 py-3 font-semibold text-muted-600">المزود</th>
                  <th className="text-right px-4 py-3 font-semibold text-muted-600 hidden md:table-cell">التخصص</th>
                  <th className="text-right px-4 py-3 font-semibold text-muted-600 hidden lg:table-cell">المدن</th>
                  <th className="text-right px-4 py-3 font-semibold text-muted-600 hidden sm:table-cell">التقييم</th>
              ...
              <tbody className="divide-y divide-muted-50">
                {services.map((s) => (
                  <tr
                    ...
                    className={`cursor-pointer hover:bg-muted-50 transition-colors ${selected?.id === s.id ? "bg-primary/5" : ""}`}
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-medium text-ink leading-tight">{s.user_name}</p>
                          <p className="text-caption text-muted line-clamp-1">{s.title}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 hidden md:table-cell">
                      {s.category ? (
                        <span className="inline-flex items-center gap-1.5">
                          <span className="inline-flex items-center justify-center w-6 h-6 rounded-md bg-muted-100 text-muted-600">
                    ...
                    <td className="px-4 py-3 text-caption text-muted-500 hidden lg:table-cell">
                      {s.cities_names.slice(0, 2).join("، ")}
                      {s.cities_names.length > 2 && ` +${s.cities_names.length - 2}`}
                    </td>
                    <td className="px-4 py-3 hidden sm:table-cell">
                      <span className="flex items-center gap-1 text-body font-medium text-muted-700">
                        <Star className="h-3.5 w-3.5 text-gold" />
                        {s.average_rating?.toFixed(1) ?? "—"}
                        <span className="text-caption text-muted">({s.reviews_count})</span>
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      {s.is_active
        ...
        {selected && (
          <div className="w-72 shrink-0 bg-white rounded-2xl card-shadow p-5 self-start sticky top-6">
            ...
            <div className="flex flex-col items-center text-center mb-5">
              ...
              <p className="text-caption text-muted-500 mt-0.5">{selected.title}</p>
              {selected.category && (
            ...
            <div className="grid grid-cols-3 gap-2 mb-4">
              <div className="bg-muted-50 rounded-xl p-2 text-center">
                <p className="text-body-lg font-bold text-ink">{selected.experience_years}</p>
                <p className="text-caption text-muted-500">سنة خبرة</p>
              </div>
              <div className="bg-muted-50 rounded-xl p-2 text-center">
                <p className="text-body-lg font-bold text-ink">{selected.average_rating?.toFixed(1) ?? "—"}</p>
                <p className="text-caption text-muted-500">تقييم</p>
              </div>
              <div className="bg-muted-50 rounded-xl p-2 text-center">
                <p className="text-body-lg font-bold text-ink">{selected.reviews_count}</p>
                <p className="text-caption text-muted-500">تقييم</p>
              </div>
            </div>
            ...
            {selected.cities_names.length > 0 && (
              <div className="mb-4">
                <p className="text-caption font-semibold text-muted-600 mb-1.5">المدن</p>
                <div className="flex flex-wrap gap-1">
                  {selected.cities_names.map((c) => (
                    <span key={c} className="text-caption bg-muted-100 text-muted-600 px-2 py-0.5 rounded-full">{c}</span>
                  ))}
                </div>
              </div>
            )}
