        ...
        <div className="flex-1 bg-white rounded-2xl card-shadow overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-muted">
              ...
              <p className="text-body">لا يوجد مستخدمون</p>
            </div>
          ) : (
            <table className="w-full text-body">
              <thead className="bg-muted-50 border-b border-muted-100">
                <tr>
                  ...
                  <th className="text-right px-4 py-3 font-semibold text-muted-600 hidden md:table-cell">الهاتف</th>
                  {/* الدولة في القائمة نفسها: بناء حملة موجّهة يبدأ بمسحٍ سريع
                      للتوزيع، لا بفتح كل مستخدم على حدة. */}
                  <th className="text-right px-4 py-3 font-semibold text-muted-600 hidden lg:table-cell">الدولة</th>
                  <th className="text-right px-4 py-3 font-semibold text-muted-600">الدور</th>
              ...
              <tbody className="divide-y divide-muted-50">
                {users.map((u) => (
                  <tr
                    ...
                    className={`cursor-pointer hover:bg-muted-50 transition-colors ${selected?.id === u.id ? "bg-primary/5" : ""}`}
                  >
                    ...
                    <td className="px-4 py-3 text-muted-500 hidden md:table-cell" dir="ltr">{u.phone}</td>
        ...
        {selected && (
          <div className="w-80 shrink-0 bg-white rounded-2xl card-shadow p-5 self-start sticky top-6">
            ...
            <div className="flex flex-col items-center text-center mb-5">
              ...
              <h2 className="font-bold text-ink">{selected.full_name}</h2>
              <p className="text-body text-muted-500" dir="ltr">{selected.phone}</p>
              {selected.city_name && <p className="text-caption text-muted mt-0.5">{selected.city_name}</p>}
            </div>

            {/* Stats */}
            <div className="grid grid-cols-2 gap-2 mb-5">
              <div className="bg-muted-50 rounded-xl p-2.5 text-center">
                <p className="text-h3 font-bold text-ink">{selected.properties_count}</p>
                <p className="text-caption text-muted-500">عقار</p>
              </div>
              <div className="bg-muted-50 rounded-xl p-2.5 text-center">
                <p className="text-h3 font-bold text-ink">{selected.average_rating ?? "—"}</p>
            ...
            {/* Role control */}
            <div className="mb-4">
              <p className="text-caption font-semibold text-muted-600 mb-1.5">الدور</p>
