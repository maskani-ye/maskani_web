...
/**
 * محتوى الشريط الجانبي — مكوّن مستقلّ على مستوى الوحدة لا دالة داخل التخطيط.
 *
 * تعريفه داخل `AdminLayout` كان ينتج **نوع مكوّن جديداً مع كل تصيير**، فتُفكّك
 * شجرته وتُبنى من جديد ويضيع موضع تمريره كلّما تغيّرت أي حالة في التخطيط.
 *
 * ⚠️ يُرسَم **مرّتين**: الشريط الثابت على الشاشات الكبيرة، ودرج الجوال الذي يبقى
 * في الشجرة دائماً (يُخفى بالإزاحة لا بالإزالة). لذلك تعيش مراجع التمرير هنا —
 * نسخةٌ لكل شريط — بدل مراجع مشتركة كانت تُكتب بعناصر الدرج المخفيّ فيبقى
 * الشريط الحقيقيّ في أعلاه مهما فعلنا.
 */
function SidebarInner({
  pathname, activeHref, user, onNavigate, onLogout,
}: {
  pathname: string;
  activeHref?: string;
  ...
  // زرّ الصفحة الحالية يبقى في وسط المرأى بعد كل انتقال.
  ...
  return (
    <div className="flex min-h-screen bg-cream" dir="rtl">
      ...
      <Drawer open={drawerOpen} onClose={() => setDrawerOpen(false)} side="right" className="!bg-white lg:hidden">
        ...
        <SidebarInner pathname={pathname} activeHref={activeItem?.href} user={user}
