...
function ownerId(u: Listing["user"]): number | null {
...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (ownerId(data.user) !== user.id) {
        toast.error("لا يمكنك تعديل إعلان لا تملكه");
  ...
  }, [authLoading, user, id, loadListing, router]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.patch(`/listings/${id}/update/`, buildListingPatch(form));
  ...
  const handleAddImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of Array.from(e.target.files)) {
        ...
        const { data } = await api.post<ListingImage[]>(`/listings/${id}/images/add/`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        // الخادم يعيد قائمة بالصور المُنشأة (getlist('images'))
        setListingImages((p) => [...p, ...(Array.isArray(data) ? data : [data])]);
      }
  ...
  const handleDeleteImage = async (imgId: number) => {
    try {
      await api.delete(`/listings/${id}/images/${imgId}/delete/`);
      setListingImages((p) => p.filter((im) => im.id !== imgId));
  ...
  const handleSetMain = async (imgId: number) => {
    try {
      await api.post(`/listings/${id}/images/${imgId}/set-main/`);
      setListingImages((p) => p.map((im) => ({ ...im, is_main: im.id === imgId })));
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        ...
        <Link href={`/listings/${id}`} className="hover:text-primary">التفاصيل</Link>
        <AltArrowRight className="h-3.5 w-3.5" />
        <span className="text-gray-700 font-medium">تعديل</span>
      </div>

      <h1 className="text-2xl font-bold text-gray-900 mb-6">تعديل الإعلان</h1>
      ...
      <div className="bg-white rounded-3xl card-shadow p-6 mt-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-gray-800">صور الإعلان</h2>
