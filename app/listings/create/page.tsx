...
import {
  emptyListingForm, buildListingFormData, type ListingFormState,
...
const AMENITIES: { key: keyof ListingFormState; label: string }[] = [
...
export default function CreateListingPage() {
  ...
  function validateStep(): boolean {
    if (step === 0) {
      if (!form.title.trim()) return fail("يرجى إدخال عنوان الإعلان");
      if (!form.property_type) return fail("يرجى اختيار نوع العقار");
      if (!form.price.trim() || isNaN(priceNum)) return fail("يرجى إدخال سعر صحيح");
    } else if (step === 1) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
      <h1 className="text-xl font-bold text-gray-900 mb-5">إضافة إعلان جديد</h1>
      ...
      <div className="bg-white rounded-3xl card-shadow p-5 sm:p-6 mt-4">
        {step === 0 && (
          <StepBasics
            ...
            currency={currency} priceNum={priceNum}
      ...
      <div className="flex items-center gap-3 mt-5">
        ...
        <Button type="button" onClick={onPrimary} loading={saving && step === LAST_STEP} fullWidth size="lg">
          {step === LAST_STEP ? "نشر الإعلان" : "التالي"}
        </Button>
      </div>
    </div>
  );
...
// ─── ① الأساسيات ─────────────────────────────────────────────────────────────
function StepBasics({
  form, setField, propertyTypes, currency, priceNum, showFurnishing, isLand, detailsOpen, setDetailsOpen,
}: {
  form: ListingFormState;
  setField: <K extends keyof ListingFormState>(k: K, v: ListingFormState[K]) => void;
  ...
  return (
    <div>
      ...
      <div className="h-4" />
      <SectionLabel text="عنوان الإعلان" required />
      <input
        className={inputCls}
      ...
      <div className="h-4" />
      <SectionLabel text="السعر" required />
      <div className="relative">
        ...
        />
        <span className="absolute left-4 top-1/2 -translate-y-1/2 text-sm text-gray-400">{currency.symbol}</span>
      </div>
      ...
      {!isNaN(priceNum) && priceNum > 0 && (
        <p className="text-base font-bold text-primary mt-2">{formatPrice(priceNum, currency.value)}</p>
      )}
...
function StepMedia({
  ...
  return (
    <div>
      ...
      <p className="text-xs text-gray-400 -mt-1 mb-2.5">الصورة الأولى ستكون الصورة الرئيسية للإعلان</p>
      <div className="flex flex-wrap gap-2">
        {images.map((img, i) => (
