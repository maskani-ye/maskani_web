...
export async function generateMetadata(
  ...
  // ⚠️ **الرقم في الوصف لا في العنوان.** كان العنوان «… كم متر مربّع؟ = ٤٤٫٤٤ م²»
  // فيقرأ الباحث جوابه في صفحة النتائج ولا ينقر: صفحات هذه الأداة جلبت ٤١٪ من
  // ظهور الموقع كلّه (١٨ ألفاً في ٢٨ يوماً) بنسبة نقر ٠٫٢٧٪ فقط. العنوان يعِد
  // الآن بما لا يسعه سطر النتائج — الحاسبة وجدول التحويل — والرقم أوّل الوصف
  // كي تبقى الصفحة جواباً صريحاً لسؤاله.
  const title = `${u.name} كم متر مربّع${where}؟ حاسبة تحويل وجدول بالمساحات`;
  const description = `${u.name}${where} = ${fmt(u.m2)} متر مربّع (${BASIS_LABEL[u.basis]})${u.region ? ` — ${u.region}` : ""}. احسب أرضك بالضبط: حوّل أي مساحة بين ${u.short} والمتر والهكتار والدونم والفدان، مع جدول جاهز — مجّاناً من مسكني.`;
  return {
    title,
    description,
    keywords: [
