...
## بنية الأسواق — كل دولة موقعٌ قائم بذاته

⚠️ **العنوان يملك الدولة، لا التخزين المحلّي.** كان الزائر يختار اليمن فيُفتح
`/ye`، ثم يبدّل من داخل الشاشات فيبقى العنوان `/ye` والمحتوى سعوديّاً — رابطٌ
يكذب على من يُشارَك معه، وصفحةٌ لا يعرف جوجل لأيّ سوق يفهرسها.

- `/` = **بوّابة عالمية** (`app/page.tsx` + `LandingClient`): شاشة واحدة بلا
  تمرير، اختيار السوق هو فعلها الوحيد. مفهرسة (كانت ترث `noindex` من صفحة
  السوق فخرجت من فهرس جوجل — 2026‑08‑30).
- `/<code>` = واجهة السوق · `/<code>/{properties,services,requests,jobs}` =
  أقسامه (`app/[market]/…`)، ونصّ كل صفحة يحمل **اسم سوقها ومدنها من الـAPI**
  (كانت تكتب «في اليمن» حرفياً لكل الأسواق).
- `app/[market]/layout.tsx` يتحقّق من الرمز، و`MarketSync` يفرض سوق المسار على
  `CountryContext` ويكتب كعكة `mk_market`.
- **مبدّل الدولة في الناف بار يفتح صفحة جديدة**: من `/ye/services` إلى
  `/sa/services` — لا تبديل حالة في مكانه. فلا فلتر دولة داخل الأقسام.
- الروابط القديمة `/properties` وأخواتها → **308** إلى `/<كعكة السوق أو ye>/…`.
- `lib/markets.ts` مصدر واحد للرموز (`MARKETS` · `marketPath` · `splitMarket`).
